...
//------------------------------------------------------------------------------
// void print_buf() -
//        from: http://stackoverflow.com/questions/10599068/how-do-i-print-bytes-as-hexadecimal
//------------------------------------------------------------------------------
void DNSDistGcaMisc::print_buf(const char *title, const unsigned char *buf, size_t buf_len)
{
    size_t i = 0;
    fprintf(stdout, "%s\n", title);
    for(i = 0; i < buf_len; ++i)
       {
        unsigned char cx = buf[i];
        if((cx <= ' ') || (cx >= 127))
          {
           cx = ' ';
          }
        fprintf(stdout, "%02X-%c%s", buf[i], cx, ( i + 1 ) % 16 == 0 ? "\r\n" : " " );
       }
    fprintf(stdout, "\n");
...
int iStatus = 0;
    ...
    iStatus = dumpDNS((const unsigned char *) value.c_str(), len);
    return(iStatus);
#ifdef TRASH
...
//------------------------------------------------------------------------------
// dumpDNS() - dump the dns answer from the cache
//------------------------------------------------------------------------------
int  DNSDistGcaMisc::dumpDNS(const unsigned char *value, uint16_t len)
{
int iStatus = 0;


    DNSDistGcaMisc::print_buf("dumpDNS", value, len);           // byte dump

      struct DNS_HEADER *dns = NULL;
      dns = (struct DNS_HEADER*) value;
...
//      reader = &buf[sizeof(struct DNS_HEADER) + (strlen((const char*)qname)+1) + sizeof(struct QUESTION)];
      ...
      buf = (unsigned char *) value;             // buf pts to the start of the dns query / response area

      qname = (unsigned char*) value;            // query pts to the query portion
      qname += sizeof(struct DNS_HEADER);                //

      reader = (unsigned char*) value;           // reader pts to past the dns header and the query field
      reader += sizeof(struct DNS_HEADER);               //
      reader += (strlen((const char*)qname)+1);          //
      reader += sizeof(struct QUESTION);                 //
    ...
    for(i=0;i<ntohs(dns->add_count);i++)
    {
        ...
        printf("-------------> dumpDNS() -> additional name: %s   length: %lu   resource type: %d \n", addit[i].name, strlen((const char *)addit[i].name), ntohs(addit[i].resource->type));
        ...
        switch(ntohs(addit[i].resource->type))
          {
           ...
           case 41:                                 // edns
                printf("OPT (41)   ");
                printf("TTL: %u    ", addit[i].resource->ttl);
                printf("Class: %u   ", addit[i].resource->_class);
                break;
           default:
                printf("OTHER (%d)   ", ntohs(addit[i].resource->type));
                break;
          }
        printf("Name: %s ",addit[i].name);
        if(ntohs(addit[i].resource->type)==1)
