...
//------------------------------------------------------------------------------
  ...
  int  findByNameXXX(const DNSName& name, uint16_t qtype=QType::ANY, bool suffixMatch=false);
  void insertEntryXXX(const DNSName& name, uint16_t qtype=QType::ANY, bool suffixMatch=false);
  static uint32_t getKeyXXX(const DNSName& qname, uint16_t consumed, const unsigned char* packet, uint16_t packetLen, bool tcp);
